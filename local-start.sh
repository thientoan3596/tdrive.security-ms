...
#==============================================================================
usage() {
  echo "Usage: $0 [-p|--pasword DB_PASSWORD] [--init-templates PATH] [--no-init-template] [--mounted-drive PATH] [--generate-password|-gp] [--new-secret] [--fresh-run|-fr] [-l]"
  ...
  echo "  --new-secret            Generate new JWT SECRET"
...
if [ "${NEW_SECRET_MODE}" = false ]; then
  JWT_SECRET=$(openssl rand -base64 32 | tr -d "\n")
  log "New jwt secret genrated [${JWT_SECRET}]"
...
if ! grep -q "^JWT_SECRET=" .env; then
  JWT_SECRET=$(openssl rand -base64 32 | tr -d "\n")
  echo "WARNING! Missing jwt secret genrated one [${JWT_SECRET}]"
  if grep -q "^JWT_SECRET=" .env; then
    sed -i 's/^JWT_SECRET=.*/JWT_SECRET='"$JWT_SECRET"'/' .env
  else
    echo "JWT_SECRET=${JWT_SECRET}" >>.env
